...
try {
  ...
  await run(["bunx", "bun2nix", "--lock-file", bunLockPath, "--output-file", bunNixPath]);
  ...
  const hash = await run(["nix", "hash", "to-sri", "--type", "sha256", prefetchHash.split("\n")[0]]);
